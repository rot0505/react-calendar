export const validationMethodsWithoutParams = {
  ...
  isNumberWithColon(value: string) {
    ...
    return !regex.test(value)
      ? { status: false, errorMessage: "The field must contains number with colon" }
      : { status: true };
  },
...
export const validationMethodsWithParams = {
  minLength(value: string, minLength: number) {
    return value.length < minLength
      ? { status: false, errorMessage: `The field must contains at least ${minLength} characters` }
      : { status: true };
  },
  maxLength(value: string, maxLength: number) {
    return value.length > maxLength
      ? { status: false, errorMessage: `The field must contains no more than ${maxLength} characters` }
      : { status: true };
  },
...
export const validationMethodsWithDates = {
  isDateInFeature(date1: Date, date2: Date) {
    return date2.getTime() - date1.getTime() > 0
      ? { status: false, errorMessage: "the end date must be later than the start date" }
      : { status: true };
  },
};
