...
const ColorPicker: React.FC<IColorPickerProps> = ({ selectedColor, onChangeColor }) => {
  ...
  return (
    <div className={styles.color_picker} ref={colorPickerRef}>
      <div className={styles.color_picker_header} onClick={toggling}>
        <div className={styles.color_picker_selected_color} style={{ background: selectedColor }} />
      ...
      {isOpen && (
        <ul className={styles.color_picker_list}>
          {colors.map((color, indx) => (
            <ColorOption key={indx} color={color} selectedColor={selectedColor} onChangeColor={onChangeColor} />
          ))}
        </ul>
      )}
    </div>
